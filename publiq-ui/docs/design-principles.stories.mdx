...
We use Bootstrap components underneath as implementation for our own custom components, so we can recreate the same look
and feel in Vue as we had in AngularJS.

Every Bootstrap component we use in Vue should be wrapped in a custom component inside Publiq UI.
...
Font sizes should be configured once globally in the Bootstrap variables and then not overridden in specific components.

## Scoped

Each component's styles should be `scoped` so the styling doesn't cascade down to child components, or pollute the global styling.
