const IconFixedDays = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="50" viewBox="0 0 64 64">
      <path
        fill="#f0f0f0"
        d="M59 13a4 4 0 0 0-4-4h-5v3a3 3 0 0 1-6 0V9H34v3a3 3 0 0 1-6 0V9H18v3a3 3 0 0 1-6 0V9H7a4 4 0 0 0-4 4v8h56z"
      />
      <circle cx="49" cy="49" r="2" fill="#f0f0f0" />
      <rect width="6" height="5" x="22" y="27" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="34" y="27" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="34" y="38" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="10" y="27" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="46" y="27" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="22" y="38" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="10" y="38" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="22" y="49" fill="#f0f0f0" rx="1" />
      <rect width="6" height="5" x="10" y="49" fill="#f0f0f0" rx="1" />
      <g fill="#777">
        <path d="M55 8h-4V6a4 4 0 0 0-8 0v2h-8V6a4 4 0 0 0-8 0v2h-8V6a4 4 0 0 0-8 0v2H7a5.006 5.006 0 0 0-5 5v42a5.006 5.006 0 0 0 5 5h30a1 1 0 0 0 0-2H7a3 3 0 0 1-3-3V22h54v14a1 1 0 0 0 2 0V13a5.006 5.006 0 0 0-5-5zM45 6a2 2 0 0 1 4 0v6a2 2 0 0 1-4 0zM29 6a2 2 0 0 1 4 0v6a2 2 0 0 1-4 0zM13 6a2 2 0 0 1 4 0v6a2 2 0 0 1-4 0zM4 20v-7a3 3 0 0 1 3-3h4v2a4 4 0 0 0 8 0v-2h8v2a4 4 0 0 0 8 0v-2h8v2a4 4 0 0 0 8 0v-2h4a3 3 0 0 1 3 3v7z" />
        <path d="M61 41a1 1 0 0 0 0-2h-4a1 1 0 0 0-1 1v4a1 1 0 0 0 2 0v-1.307a10.975 10.975 0 1 1-5.162-4 1 1 0 1 0 .7-1.875A13.018 13.018 0 1 0 59.235 41z" />
        <path d="M50 46.184V41a1 1 0 0 0-2 0v5.184A3 3 0 0 0 46.184 48H43a1 1 0 0 0 0 2h3.184A2.993 2.993 0 1 0 50 46.184zM49 50a1 1 0 1 1 1-1 1 1 0 0 1-1 1zM27 33h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM39 33h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM15 33h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM51 33h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM27 44h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM35 44a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3a1 1 0 0 1 0 2h-3v3a1 1 0 0 1 0 2zM15 44h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM27 55h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3zM15 55h-4a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2zm-4-5v3h4v-3z" />
      </g>
    </svg>
  );
};

export { IconFixedDays };
